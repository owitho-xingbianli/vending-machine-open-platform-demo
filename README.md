...
### accessToken获取:
```
 * accessToken是第三⽅平台的全局唯一票据，第三方调⽤各接口时都需使用accessToken。第三方需要进⾏行妥善保存。
 * accessToken的存储至少要保留512个字符空间。accessToken的有效期目前为24个小时，需定时刷新，重复获取将导致上次获取的accessToken失效。
 * 开放平台以accessToken为接⼝调⽤凭据，来调用接口，所有接口的调用需要先获取accessToken，accessToken在24⼩时内有效，过期需要重新获取，但一天内获取的次数有限，开发者需要⾃自⾏行存储。
```
...
```
 	...
 	2、目前accessToken的有效期通过返回的expire_time来传达，目前是24⼩时。中控服务器需要根据这个有效时间提前去刷新新accessToken。在刷新过程中，中控服务器对外输出的依然是老
accessToken，此时开放平台后台会保证在刷新短时间内，新老accessToken都可用，这保证了第三⽅业务的平滑过渡;
...
```
{
    ...
    "salt":1234,
    "utc":1526697000000,
    "signature":"5fff5b80aeeef36e1c066b1004ec0262"
}
```
```
salt为1000-9999随机数
...
```
appId=4cdbc040657a4847b2667e31d9e2c3d9&accessToken=9e7d0604-cb0d-4c7c-a09d-cf365ce6936c&data=%7B%22name%22%3A%22%E5%BC%A0%E4%B8%89%22%2C%22age%22%3A23%2C%22country%22%3A%22%E4%B8%AD%E5%9B%BD%22%7D&salt=1234%26utc%3D1526697000000
```
...
|payStatus|Int|Y|支付状态|0:待支付 1:支付成功 2:支付失败[详见附录](#机器状态)|
...
| outStatus |int|Y|-|出货状态|200:出货成功 300:部分成功 400:出货失败 [详见附录](#状态字典)|
| outDetailList |Array|Y|-|出货详情|出货详情根据订单按单个商品平铺展开|
| slotNum|string|Y|-|货道号|
| skuCode|string|Y|-|商品id|
| status|int|Y|-|每个商品的出货状态|100:出货中 200:出货成功 300:部分成功 400:出货失败 [详见附录](#状态字典)|
...
| outStatus |int|出货状态|100:出货中 200:出货成功 300:部分成功 400:出货失败 [详见附录](#状态字典)|
